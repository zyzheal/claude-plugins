...
function getCpuStateForPid(pid) {
    try {
...
function setCpuStateForPid(pid, cpuTimeSec, wallMs) {
    try {
 ...
 */
function computeCpuPercent(claudePid) {
 ...
 */
function findClaudeCodePid(startPid) {
...
function getResourceMacOSFallback(pid) {
    try {
...
function findClaudeCodePidLinux(startPid) {
    let current = startPid;
    const visited = new Set();
    while (current > 0 && !visited.has(current)) {
        visited.add(current);
...
function getChildrenLinux(parentPid, result) {
    try {
