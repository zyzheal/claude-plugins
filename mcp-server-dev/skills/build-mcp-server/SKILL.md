...
- If the SDK throws on elicitation: the client hasn't advertised the capability -- check `clientCapabilities.elicitation` and fall back to plain text/JSON
- If tool descriptions are too vague for Claude to use effectively: follow `references/tool-design.md` -- each description should name the action, its inputs, and what it returns
- If FastMCP 3.x import fails: confirm it's jlowin's `fastmcp` package (`pip install fastmcp`), NOT the frozen FastMCP 1.x bundled in the official `mcp` SDK
- If OAuth setup is needed but the server is local stdio: OAuth requires a remote HTTP server with redirect handling -- switch deployment model or use API key auth
- If the context window floods with tools: switch from "one tool per action" to "search + execute" pattern, or promote only the 3-5 most-used actions to dedicated tools
- If `mcp-remote` SSE probe swallows widget capability negotiation: force `--transport http-only` when using mcp-remote as a proxy
