...
- If the widget renders blank: open the iframe's devtools console -- CSP violations are the #1 cause; the `/*__EXT_APPS_BUNDLE__*/` inline pattern is mandatory because the iframe CSP blocks CDN fetches
- If `registerAppTool` or `registerAppResource` throws: verify the URI scheme matches between the tool's `_meta.ui.resourceUri` and the resource registration
- If `app.connect()` hangs: check that the host supports the apps surface; test with a host that doesn't support it to confirm graceful degradation
- If Desktop caches UI resources: fully quit (Cmd+Q / Alt+F4, not window-close) and relaunch to force a cold re-fetch
- If `app.openLink()` is missing: the sandbox blocks `window.open` and `<a target="_blank">` -- always use `app.openLink({url})` for outbound navigation
- If the widget works in Claude but not ChatGPT (or vice versa): verify the widget doesn't use host-specific APIs; the `App` class is transport-agnostic
