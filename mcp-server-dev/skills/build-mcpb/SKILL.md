...
- If `mcpb pack` fails: check that `manifest.json` exists at the root and passes `mcpb validate` first
- If bundled dependencies include native modules (C++ addons): build for each target platform separately, or avoid native deps entirely
- If the server works on dev machine but fails after packaging: test on a clean machine without the dev toolchain -- "works on my machine" failures almost always trace to unbundled dependencies
- If `${user_config.*}` substitution doesn't work: verify the key names in `user_config` match exactly what you reference in `server.mcp_config.env`
- If the MCPB installs but doesn't launch: check that `server.mcp_config.command` and `args` use `${__dirname}` correctly for bundle-relative paths
