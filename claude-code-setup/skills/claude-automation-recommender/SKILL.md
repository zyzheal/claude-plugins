...
- If project type detection is ambiguous: present the most likely options and ask the user to confirm
- If no existing Claude Code config is found: recommend starting with hooks (lowest risk, highest immediate value)
- If a recommended MCP server requires external setup (OAuth, API keys): note the prerequisite clearly and provide setup steps
- If the codebase is too small for meaningful recommendations: focus on general-purpose automations (formatting, linting, commit hooks)
