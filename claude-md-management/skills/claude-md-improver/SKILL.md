...
| 场景 | 触发条件 | 处理动作 |
|------|----------|----------|
| CLAUDE.md 不存在 | 项目中无 CLAUDE.md 文件 | 创建初始模板，引导用户填写 |
| CLAUDE.md 过大 | 文件超过 50 行 | 建议拆分为多个子文档 |
| 内容过时 | 包含已废弃的命令或配置 | 标记为过时，建议更新 |
| 通用建议过多 | 大量 generic advice | 删除无用的泛泛之谈，保留项目特定信息 |
| 格式不规范 | 缺少章节标题或结构混乱 | 重新组织为标准章节格式 |
